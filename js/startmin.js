...
/*PROPS*/
    ...
    var tbl_processed = $('#tbl_processed').DataTable(
      { "bPaginate": true,
        "bFilter": true,
        "bInfo": true,
      });
    var tbl_delivered = $('#tbl_delivered').DataTable(
      { "bPaginate": true,
        "bFilter": true,
        "bInfo": true,
      });
          ...
          .on("child_added",function(snap){    
                  if(snap.exists()) {
                    ...
                    switch(status){
                      ...
                      case 'request':tbl_request.rows.add([{0:snap.key,1:snap.val().userData}]).draw();
                                    break;

                      case 'processed':tbl_processed.rows.add([{0:snap.key,1:snap.val().userData}]).draw();
                                    break;

                      case 'delivered':tbl_delivered.rows.add([{0:snap.key,1:snap.val().userData}]).draw();
                                    break;

                      case 'canceled':tbl_cancelled.rows.add([{0:snap.key,1:snap.val().userData}]).draw();
                                    break;
                    }
          ...
          .on("value",function(snap){ 
             if(snap.exists()) {
                ...
                switch(node){
                  ...
                  case "purchase": 
                                  if ($('#pl_carrito').children().length > 0 ) $("#pl_carrito").empty();
                  ...
                  case "shipping": 
                                   body="<ul>"+
                                                ...
                                                "<li><b>Cantón:</b> "+data.zip+"</li>"+
                                                "<li><b>Dirección:</b> "+data.address+"</li>"+
                                                "<li><b>Mensajero:</b> "+data.messenger+"</li>"+
                                                "<li><b>Costo:</b> "+data.cost+"</li>"+
                                          "</ul>";
                                   drawPanels(body,id,"pl_destination");
                                   break;
                  case "users":  
                                    body="<ul>"+
                                                "<li><b>Nombre:</b> "+data.name+" "+data.lastName+"</li>"+
                                                "<li><b>Email:</b> "+data.email+"</li>"+
                                                "<li><b>Tel:</b> "+data.phone+"</li>"+
                                          "</ul>";
   ...
   */function _onChangeStatusPage(catalog){
      ...
      //CLICKS LISTENER                                        
        ...
        $('#btn_orderProcessed').click(()=>{
          ...
          loadOrder("processed");
          showPage("tbl_orders",true);
          showPage("tbl_processeds",true);
        });
        $('#btn_orderDelivered').click(()=>{
          tbl_delivered.clear();
          loadOrder("delivered");
          showPage("tbl_orders",true);
          showPage("tbl_delivereds",true);
        });
        $('#btn_orderCanceled').click(()=>{
          tbl_cancelled.clear();
          loadOrder("canceled");
          showPage("tbl_orders",true);
          showPage("tbl_cancelleds",true);
        });
      ...
      //METHODS TABLE
        ...
        _captureRowData("tbl_request",'modalOrder',false);//
        _captureRowData("tbl_processed",'modalOrder',false);//
        _captureRowData("tbl_delivered",'modalOrder',false);//
        _captureRowData("tbl_cancelled",'modalOrder',false);//
...
// --------------- NAVEGATION DASHBOARD ---------------
   ...
   */const showPage=(id="tbl_orders",display="true")=>{//frm || tbl 
        ...
        $("#tbl_requests").css("display", "none");
        $("#tbl_processeds").css("display", "none");
        $("#tbl_delivereds").css("display", "none");
        $("#tbl_cancelleds").css("display", "none");
        if(id == "tbl_products" || id == "frm_products"){
          $("#pls_orders").css("display", "none");
          $("#tbl_orders").css("display", "none");
        }
        if(id == "tbl_orders")$("#pls_orders").css("display", "none");
        if(id == "pls_orders")$("#tbl_orders").css("display", "none");

        let page =$("#"+id);
        page.css("display", display?"display":"none");
        display?page.fadeIn(300):page.fadeOut(300);
        screen =id;
